...
if aba in ["🌲 Florestas", "🌊 Oceanos", "🏳️ Países"]:
    if aba == "🌲 Florestas":
        ...
        lista_loc = ["Portugal","Brasil","EUA","China","Japão","Índia","França","Alemanha","Itália","México",
                     "Rússia","Argentina","Egito","África do Sul","Marrocos","Turquia","Irã","Arábia Saudita",
...
# ----------------------
    ...
    if query:
        try:
            r = requests.get(f"https://api.inaturalist.org/v1/taxa?q={query}&taxon_id=1&per_page=70&locale=pt-PT", timeout=5)
            results = r.json().get("results", [])
        except:
            results = []
        grid(results, "lab")
...
# ----------------------
# DEMÁS ABAS (SALVAMENTO, VETERINÁRIO, TANQUE, ZOO, DEF)
# ----------------------
# Aqui você mantém todo o código antigo do seu app exatamente como estava
# Sem alterações, conforme sua instrução.
