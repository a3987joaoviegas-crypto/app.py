...
# 4. DESIGN CSS (BORDAS ARCO-ÍRIS APENAS NA BORDA)
...
if is_mega:
    # Apenas a borda anima, o conteúdo (filtro) é removido
...
<style>
    ...
    /* Garante que o animal não muda de cor nem fica P&B */
    .img-animal {{ 
        width: 100%; 
        height: 220px; 
        border-radius: 12px; 
...
# 5. MOTOR DE BUSCA (70 RESULTADOS)
def buscar_70(q):
    try:
...
def render_cartao(an, prefixo, i, habitat):
    ...
    <div class='cartao-cidadao'>
        ...
        <h4 style='margin:10px 0 5px 0;'>{an['nome']}</h4>
...
if aba == T['paises']:
    p = st.selectbox("País", ["Portugal", "Brasil", "Japão", "Austrália", "Angola", "India", "Canada", "USA"])
    grid_3(p, "pa", f"Região de {p}")

elif aba == T['florestas']:
    h = {"Amazónia": "Amazon Forest", "Savana": "Savanna", "Deserto": "Desert", "Selva Tropical": "Rainforest", "Pantanal": "Pantanal"}
    f_sel = st.selectbox("Habitat", list(h.keys()))
    grid_3(h[f_sel], "fl", f_sel)

elif aba == T['oceanos']:
    m = {"Recife de Coral": "Coral Reef", "Oceano Profundo": "Deep Sea", "Mar Mediterrâneo": "Mediterranean Sea", "Mar Vermelho": "Red Sea"}
