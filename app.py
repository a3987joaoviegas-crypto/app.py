...
# 2. DICIONÁRIO DE TRADUÇÃO TOTAL
idiomas = {
    ...
    "Russo (Русский)": {"paises": "Страны", "florestas": "Леса", "oceanos": "Океаны", "lab": "Лаборатория", "col": "Коллекция", "def": "Настройки", "luta": "Бой", "arena": "Боевая ареna", "guardar": "Сохранить", "grupo": "Фильтр группы"},
...
st.markdown(f"""
    ...
    .cc-card {{ background: {c_bg} !important; border-radius: 15px; padding: 20px; border-left: 15px solid gold; box-shadow: 10px 10px 20px rgba(0,0,0,0.5); margin-bottom: 20px; }}
...
def render_cartao(an, k, l_btn="➕"):
    st.markdown(f"""
    <div class='cc-card'>
        <img src='{an['foto']}' style='width:100%; border-radius:10px; height:250px; object-fit:cover;'>
        <h2 style='margin-bottom:0;'>{an['nome']}</h2>
        <i style='opacity:0.7;'>{an['sci']}</i>
    </div>
    """, unsafe_allow_html=True)
    if st.button(l_btn, key=k, use_container_width=True):
        if T['luta'] in l_btn: st.error(f"LUTANDO CONTRA {an['nome']}!")
...
if f"🔬 {T['lab']}" in aba:
    ...
    with col_pesq:
        if st.session_state.arena_ativa:
            ...
            term = st.text_input("Análise Biológica:")
            if term:
                res = buscar_api(term)
                c_a, c_b = st.columns(2)
                for i, a in enumerate(res):
                    with (c_a if i % 2 == 0 else c_b): render_cartao(a, f"lab_card_{i}")
    ...
    st.image(f"https://flagcdn.com/w320/{'pt' if p=='Portugal' else 'br' if p=='Brasil' else 'ao' if p=='Angola' else 'mz' if p=='Moçambique' else 'jp' if p=='Japão' else 'au'}.png", width=100)
    res = buscar_api(f"Animais {g} de {p}")
    cols = st.columns(2)
    for i, a in enumerate(res):
        with cols[i%2]: render_cartao(a, f"p_c_{i}")
    ...
    cols = st.columns(2)
    for i, a in enumerate(res):
        with cols[i%2]: render_cartao(a, f"f_c_{i}")

elif f"🌊 {T['oceanos']}" in aba:
    st.title(T['oceanos'])
    o = st.selectbox("Região:", ["Oceano Atlântico", "Recife de Coral", "Mar Profundo"])
    g = st.selectbox(T['grupo'], GRUPOS, key="g_o")
    res = buscar_api(f"Animais {g} do {o}")
    cols = st.columns(2)
    for i, a in enumerate(res):
        with cols[i%2]: render_cartao(a, f"o_c_{i}")
    ...
    escolha = st.selectbox("🌐 Idioma / Language:", list(idiomas_dict.keys()), index=list(idiomas_dict.keys()).index(st.session_state.lang_label))
    ...
    for i, a in enumerate(st.session_state.zoo):
        ...
        if st.button(f"Libertar {i}", key=f"rel_{i}"): st.session_state.zoo.pop(i); st.rerun()
